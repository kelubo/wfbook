...
- ​							[使用 Mariabackup 的全备份和恢复](https://mariadb.com/kb/en/library/full-backup-and-restore-with-mariadb-backup/) 。 					
...
- ​							[使用 Mariabackup 的全备份和恢复](https://mariadb.com/kb/en/library/full-backup-and-restore-with-mariabackup/) 。 					
...
- ​							安装 **MariaDB Galera Cluster** 软件包。例如： 					
  ...
  - ​									`galera` 							
    ...
    ​									有关构建 **MariaDB Galera Cluster** 的组件的更多信息，请参阅 [第 2.8.2 节 “构建 MariaDB Galera 集群的组件”](https://access.redhat.com/documentation/zh-cn/red_hat_enterprise_linux/9/html-single/configuring_and_using_database_servers/index#components-to-build-mariadb-galera-cluster_replicating-mariadb-with-galera) 							
...
- ​							在系统首次添加到集群前，必须更新 **MariaDB** 服务器复制配置。 					
  ...
  - ​									对于所有其他节点，将 `wsrep_cluster_address` 设置为包含已属于正在运行的集群的一部分的任何节点的地址。例如： 							
    ...
    ​									有关如何设置 Galera 集群地址的更多信息，请参阅 [Galera Cluster Address](https://mariadb.com/kb/en/library/galera-cluster-address/)。 							
