...
​			因为一些原因，准确计时非常重要。例如在网络中，需要准确的数据包和日志的时间戳。在 Linux 系统中，`NTP` 协议是由在用户空间运行的守护进程实现的。 	
...
## 28.2. 使用 chronyc 来控制 chronyd

​				本节描述了如何使用 **chronyc** 命令行工具控制 `chronyd`。 		

**流程**

1. ​						要在互动模式中使用命令行工具 **chronyc** 来更改本地 `chronyd` 实例，以`根`用户身份输入以下命令： 				
