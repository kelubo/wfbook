# Comparison of NTP implementations NTP 实现的比较
...
| Minimum number of sources 最小来源数   | 1 (configurable) 1（可配置） | 1 (configurable) 1（可配置） | 1          |
...
| Step threshold 步进阈值                                      | Infinity (configurable) 无穷大（可配置）                     | 0.128 s (configurable) 0.128 s （可配置） | N/A                                                          |
| Limited number of steps 步骤数有限                           | Yes (configurable) 是（可配置）                              | No                                        | Yes                                                          |
| Panic threshold 恐慌阈值                                     | Infinity (configurable) 无穷大（可配置）                     | 1000 s (configurable) 1000 秒（可配置）   | N/A                                                          |
| Maximum slew rate 最大压摆率                                 | System specific (Linux: 100000 ppm, FreeBSD, NetBSD, macOS: 5000 ppm, illumos: 32500 ppm) (configurable) 特定于系统（Linux：100000 ppm，FreeBSD，NetBSD，macOS：5000 ppm，illumos：32500 ppm）（可配置） | 500 ppm 500 页/分钟                       | System specific (Linux: 500 ppm, FreeBSD, NetBSD: 5000 ppm, illumos: 65000 ppm) 特定于系统（Linux：500 ppm，FreeBSD，NetBSD：5000 ppm，illumos：65000 ppm） |
| Restore frequency from file 从文件恢复频率                   | Yes                                                          | Yes                                       | Yes                                                          |
| Limited wakeups (power saving) 有限唤醒（省电）              | Yes                                                          | No                                        | Yes                                                          |
| Temperature compensation 温度补偿                            | Yes                                                          | No                                        | No                                                           |

### NTP modes NTP 模式

|                                  | `chrony` | `ntp` | `openntpd` |
| -------------------------------- | -------- | ----- | ---------- |
| Server (mode 4) 服务器（模式 4） | Yes      | Yes   | Yes        |
| Client (mode 3) 客户端（模式 3） | Yes      | Yes   | Yes        |
| Persistent symmetric 持久对称    | Yes      | Yes   | No         |
| Ephemeral symmetric 短暂的对称   | No       | Yes   | No         |
| Broadcast server 广播服务器      | Yes      | Yes   | No         |
| Broadcast client 广播客户端      | No       | Yes   | No         |
| Multicast server 组播服务器      | No       | Yes   | No         |
| Multicast client 组播客户端      | No       | Yes   | No         |
| Manycast server Manycast 服务器  | No       | Yes   | No         |
| Manycast client Manycast 客户端  | No       | Yes   | No         |
