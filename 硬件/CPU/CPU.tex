...
欢迎来到CPU零基础入门课程！本课程专为完全没有计算机硬件基础的学习者设计，将带你从零开始了解CPU的基本概念、工作原理和实际应用。
...
\begin{enumerate}
    \item \textbf{作用}：解决CPU与内存之间的速度差异
    \item \textbf{层级结构}：
    \begin{itemize}[leftmargin=2em]
        \item L1缓存：最小、最快，集成在CPU核心内
        \item L2缓存：较大、稍慢，每个核心独立或共享
        \item L3缓存：最大、最慢，所有核心共享
    \end{itemize}
    \item \textbf{工作原理}：将常用数据暂存，减少访问内存的次数
\end{enumerate}
