...
Ansible 模块是可以控制系统资源或执行系统命令的代码单元。Ansible 提供了一个模块库，可以直接在远程主机上或通过 Playbook 执行。还可以编写自定义模块。

与模块类似的是插件，它们是扩展核心 Ansible 功能的代码片段。Ansible 使用插件架构来实现丰富、灵活和可扩展的功能集。Ansible 附带了几个插件，让您可以轻松使用自己的插件。

## 模块

Modules (also referred to as “task plugins” or “library plugins”) are discrete units of code that can be used from the command line or in a  playbook task.模块（也称为“任务插件”或“库插件”）是可以从命令行或在 playbook 任务中使用的离散代码单元。Ansible 通常在远程管理节点上执行每个模块，并收集返回值。在 Ansible 2.10 及更高版本中，大多数模块都托管在集合中。
...
每个模块都支持获取参数。Nearly all modules take `key=value` arguments, space delimited. 几乎所有模块都采用key=value参数，以空格分隔。Some modules take no arguments, and the  command/shell modules simply take the string of the command you want to  run.有些模块不接受参数，命令/shell模块只接受要运行的命令的字符串。

从 playbook 来看，Ansible 模块的执行方式非常相似。
...
向模块传递参数的另一种方法是使用YAML语法，也称为 ‘complex args’ 。
...
所有模块都返回 JSON 格式的数据。这意味着模块可以用任何编程语言编写。模块应该是幂等的，如果它们检测到当前状态与期望的最终状态匹配，则应该避免进行任何更改。When used in an Ansible playbook,  modules can trigger ‘change events’ in the form of notifying [handlers](https://docs.ansible.com/ansible/latest/playbook_guide/playbooks_handlers.html#handlers) to run additional tasks.在 Ansible playbook 中使用时，模块可以以通知处理程序运行其他任务的形式触发“更改事件”。
...
从Ansible 2.10开始，大多数模块都在集合中。每个集合的分发方法反映了对该集合中模块的维护和支持。
