...
持续集成（CI）通过自动化测试执行和趋势分析，彻底改变了公司处理构建管理、发布管理、部署自动化和测试编排的方式。

### 自动化任务定义

Jenkins 能够根据它在软件配置管理系统中识别的存储库自动创建、更新和删除作业。 Leverage this feature and optimize your job management by structuring your job definitions in a way that maximizes the benefits offered by Jenkins' automatic job management capabilities.利用此功能，以最大限度地利用 Jenkins 的自动作业管理功能提供的优势来构建作业定义，从而优化作业管理。
...
- [Use Pipeline](https://www.jenkins.io/doc/book/using/best-practices/#use-pipeline) [使用 Pipeline](https://www.jenkins.io/doc/book/using/best-practices/#use-pipeline) - 手动定义的 Pipeline 作业，以更好地控制作业管理过程。
...
如果组织文件夹不适合您，请考虑使用多分支管道作为替代方法。但是，需要强调的是，组织文件夹比多分支流水线更受青睐，因为它们能够在添加或删除存储库时自动创建和删除多分支项目。
...
请注意，根据浏览器和语言的不同，可能需要下载插件。

还可以使用 [Locale 插件](https://plugins.jenkins.io/locale) 更改系统语言（在执行构建时使用）以及所有用户的界面语言。
