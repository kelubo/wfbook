...
要创建备份，请使用 `--backup` 选项运行 xtrabackup。必须指定 `--target-dir` 选项，这是存储备份的位置。如果 InnoDB 数据或日志文件未存储在同一目录中，则必须指定它们的位置。如果目标目录不存在，则 xtrabackup 将创建该目录。如果目录存在且为空，则 xtrabackup 成功。xtrabackup succeeds.

xtrabackup 不会覆盖现有文件。它将失败，并显示操作系统错误 17，`file exists`。
...
the output shows the copied data files, and a log file thread that scans and copies from the log files.
...
备份完成后，目标目录将包含如下文件，假设有一个 InnoDB 表 `test.tbl1` 并且使用的是 MySQL 的 `innodb_file_per_table` 选项：
...
使用 `--backup` 选项创建备份后，需要准备备份，然后可用于还原。数据文件在准备好之前不是时间点一致的，因为它们是在程序运行的不同时间复制的，并且它们可能已在发生这种情况时发生更改。

如果尝试使用这些数据文件启动 InnoDB，它将检测到损坏并停止工作以避免在损坏的数据上运行。`--prepare` 步骤使文件在单个时刻完全一致，因此可以在它们上运行 InnoDB。

可以在任何计算机上运行 prepare 操作；它不需要位于原始服务器或要还原到的服务器上。可以将备份复制到实用程序服务器并在其中进行准备。

在 prepare 操作期间，xtrabackup 会启动一种修改后的嵌入式 InnoDB（xtrabackup 链接的库）。这些修改对于禁用  InnoDB 标准安全检查是必要的，例如抱怨日志文件的大小不正确。This warning is not appropriate for working with backups. 此警告不适用于处理备份。这些修改仅适用于 xtrabackup  二进制文件；不需要修改后的 InnoDB 即可使用 xtrabackup 进行备份。

The prepare step uses this “embedded InnoDB” to perform crash recovery on the copied data files, using the copied log file. 准备步骤使用此“嵌入式 InnoDB”使用复制的日志文件对复制的数据文件执行崩溃恢复。`prepare` 步骤使用起来非常简单：你只需使用 `--prepare` 选项运行 xtrabackup 并告诉它要准备哪个目录，例如，准备之前进行的备份运行：

```bash
$ xtrabackup --prepare --target-dir=/data/backups/
```

完成后，应该会看到 `InnoDB shutdown`，并显示如下消息，其中 LSN 的值将再次取决于您的系统：
...
> **注意：**
>
> 如果打算将备份作为进一步增量备份的基础，则应在准备备份时使用 `--apply-log-only` 选项，否则将无法对其应用增量备份。

## 增量备份

Xtrabackup 支持增量备份，这意味着它们可以只复制自上次备份以来更改的所有数据。

> 注意
>
> MyRocks 存储引擎上的增量备份不会确定早期的完整备份或增量备份是否包含相同的文件。Percona XtraBackup 每次备份时都会复制所有 MyRocks 文件。

可以在每个完整备份之间执行多个增量备份，因此可以设置一个备份过程，例如每周一次完整备份和每天一次增量备份，或者每天一次完整备份和每小时增量备份。

增量备份之所以有效，是因为每个 InnoDB 页面都包含一个日志序列号 （LSN）。LSN 是整个数据库的系统版本号。每个页面的 LSN 显示它最近更改的时间。

An incremental backup copies each page which LSN is newer than the previous incremental or full backup’s LSN. 增量备份会复制 LSN 比上一个增量备份或完整备份的 LSN 更新的每个页面。一个算法会查找符合条件的页面。该算法读取数据页并检查页 LSN。

### 创建增量备份

要进行增量备份，请像往常一样从完整备份开始。xtrabackup 二进制文件将名为 `xtrabackup_checkpoints` 的文件写入备份的目标目录。This file contains a line showing the `to_lsn`, which is the database’s LSN at the end of the backup. 此文件包含一行显示 `to_lsn`，即备份结束时数据库的 LSN。使用以下命令创建完整备份：

```bash
$ xtrabackup --backup --target-dir=/data/backups/base
```

如果您查看 `xtrabackup_checkpoints` 文件，应该会看到类似的内容，具体取决于您的 LSN nuber：
...
`/data/backups/inc1/` 目录现在应包含 delta 文件，例如 `ibdata1.delta` 和 `test/table1.ibd.delta`。这些表示自 `LSN 1626007` 以来的变化。如果检查此目录中的 `xtrabackup_checkpoints` 文件，应该会看到与以下内容类似的内容：
...
> 注意：
>
> you can see that there is a difference between the `to_lsn` (last checkpoint LSN) and `last_lsn` (last copied LSN), this means that there was some traffic on the server during the backup process.
...
> Warning 警告
>
> If you do not use the `--apply-log-only` option to prevent the rollback phase, After transactions have been rolled back, further  incremental backups cannot be applied.
...
要准备基本备份，需要像往常一样运行 `--prepare`，但要阻止回滚阶段：
...
日志序列号应与之前看到的基本备份的 `to_lsn` 匹配。

> Warning 警告
>
> This backup is actually safe to restore as-is now, even though the rollback  phase has been skipped. If you restore it and start MySQL, InnoDB will  detect that the rollback phase was not performed, and it will do that in the background, as it usually does for a crash recovery upon start. 
...
同样，LSN 应与之前检查第一次增量备份时看到的内容相匹配。如果从 `/data/backups/base` 还原文件，应该会看到第一次增量备份时的数据库状态。
...
1. 安装 `mysqlbackup` 组件并在服务器上启用它：
   
   ```mysql
   $ INSTALL COMPONENT "file://component_mysqlbackup";
   ```
   
1. 检查 `mysqlbackup` 组件是否安装成功：
...
可以使用 `--page-tracking` 选项为完整备份和增量备份启用页面跟踪功能。
...
Percona XtraBackup 会在内存中处理所有被跟踪页面的列表。如果 Percona XtraBackup  没有足够的可用内存来处理此列表，该进程将引发错误并退出。例如，如果增量备份使用 200GB，Percona XtraBackup 可以使用额外的 100MB 内存来处理和存储页面跟踪数据。
...
在服务器上加载并激活 mysqlbackup 组件后，可以使用以下选项手动启动页面跟踪：
...
当开始页面跟踪时，它会在服务器的数据目录下创建一个文件，以收集有关已更改页面的数据。此文件会一直增长，直到您停止页面跟踪。如果停止服务器，然后重新启动它，页面跟踪会创建一个新文件，但也会保留旧文件。旧文件会继续增长，直到您明确停止页面跟踪。
...
### Prepare a partial backup[¶](https://docs.percona.com/percona-xtrabackup/8.4/prepare-partial-backup.html#prepare-a-partial-backup) 准备部分备份
