...
磁带具有物理寿命限制（通常10-30年），需要定期将旧磁带上的数据迁移到新磁带。

### 迁移策略

| 磁带类型 | 建议迁移周期 | 迁移作业名称 | 说明 |
|---------|------------|------------|------|
| DDS4 | 5-10年 | `Copy-DDS4-Tape-Migration` | 小型磁带，建议较早迁移 |
| LTO5 | 10-15年 | `Copy-LTO5-Tape-Migration` | 独立驱动器磁带 |
| LTO5磁带库 | 10-15年 | `Copy-LTO5-Changer-Migration` | 自动选择最旧卷迁移 |

### 迁移计划

系统配置了年度迁移计划 `YearlyTapeMigration`：
- **执行时间**：每年1月1日凌晨2:00
- **迁移方式**：自动选择最旧的磁带进行复制
- **优先级**：20（较低，不影响日常备份）
...
# 查看特定池的卷
list volumes pool=Full-LTO5-Changer

# 查看卷的详细信息（包括创建时间、最后写入时间等）
list volume=LTO5-Changer-0001
```
...
```bash
# 执行磁带库迁移作业（自动选择最旧卷）
run job=Copy-LTO5-Changer-Migration yes

# 执行DDS4磁带迁移作业
run job=Copy-DDS4-Tape-Migration yes

# 执行LTO5独立磁带迁移作业
run job=Copy-LTO5-Tape-Migration yes
```
...
# 复制特定卷到新磁带
run job=Copy-LTO5-Changer-Migration
# 系统会提示选择源卷和目标卷
```
...
| 操作 | 建议 |
|------|------|
| 存储环境 | 温度18-25°C，湿度40-60%，避免阳光直射 |
| 定期读取 | 每年至少读取一次磁带，检查数据完整性 |
| 迁移周期 | DDS4: 5-10年，LTO5: 10-15年 |
| 磁带轮换 | 使用"祖父-父亲-儿子"轮换策略 |
| 异地存储 | 重要数据磁带应异地备份 |
| 标签管理 | 清晰标注磁带内容、创建日期、过期日期 |

### 注意事项

1. **迁移前备份**：确保旧磁带数据可读取后再执行迁移
2. **新磁带准备**：迁移前确保有足够的空白磁带
3. **验证数据**：迁移完成后务必验证新磁带的数据完整性
4. **保留旧磁带**：迁移成功后，建议保留旧磁带至少6个月
5. **记录日志**：记录每次迁移的详细信息（源卷、目标卷、日期）
