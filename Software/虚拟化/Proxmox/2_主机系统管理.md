...
Proxmox VE is based on [Debian GNU/Linux](https://www.debian.org/) with additional repositories to provide the Proxmox VE related packages.Proxmox VE 基于 Debian GNU/Linux 提供额外的软件包，以提供 Proxmox VE 相关的软件包。这意味着 Debian 软件包的全系列都可用，包括安全更新和错误修复。Proxmox VE 提供了自己的基于 Ubuntu 内核的 Linux 内核。它启用了所有必要的虚拟化和容器功能，并包括 ZFS 和几个额外的硬件驱动程序。

## 软件包仓库

Proxmox VE 使用 APT 作为其软件包管理工具，就像任何其它基于 Debian 的系统一样。

Proxmox VE 每天自动检查软件包更新。root@pam 用户将通过电子邮件收到有关可用更新的通知。在 GUI 中，可以使用 “ *Changelog* ” 按钮查看有关所选更新的更多详细信息。
...
> 注意：
>
> 需要有效的 Debian 和 Proxmox 存储库才能获得最新的安全更新、错误修复和新功能。

APT 存储库由 /etc/apt/sources.list 文件和放置在 /etc/apt/sources.list.d/ 目录中的 .list 文件定义。

#### 存储库管理

![](../../../Image/g/gui-node-repositories.png)

从 Proxmox VE 7 开始，可以在 Web 界面中检查存储库状态。节点摘要面板显示高级状态概述，而单独的“存储库”面板显示所有已配置存储库的深入状态和列表。
...
Proxmox VE 提供三种不同的软件包存储库。

### Proxmox VE 企业存储库

这是推荐的存储库，可供所有 Proxmox VE 订阅用户使用。它包含最稳定的封装，适用于生产用途。默认情况下，pve-enterprise 存储库处于启用状态：
...
请注意，需要有效的订阅密钥才能访问 pve-enterprise 存储库。

> 注意：
>
> 可以通过使用 #（在行的开头）注释掉上面的行来禁用此存储库。这样可以防止在主机没有订阅密钥时出现错误消息。在这种情况下，请配置 pve-no-subscription 存储库。

### Proxmox VE 无订阅存储库

顾名思义，不需要订阅密钥即可访问此存储库。它可用于测试和非生产用途。不建议在生产服务器上使用它，因为这些包并不总是经过严格的测试和验证。

建议在 /etc/apt/sources.list 中配置此存储库。
...
> Note:
>
> pvetest 存储库（顾名思义）应该仅用于测试新功能或错误修复。

### Ceph Reef Enterprise 存储库

此存储库包含企业 Proxmox VE Ceph 18.2 Reef 软件包。它们适用于生产。如果在 Proxmox VE 上运行 Ceph 客户端或完整的 Ceph 集群，请使用此存储库。
...
This Ceph repository contains the Ceph Quincy packages before they are moved to the main repository.
...
从 Debian Bookworm （Proxmox VE 8） 开始，非自由固件（由 DFSG 定义）已移至新创建的 Debian 存储库组件 non-free-firmware 。
...
为了能够从这个组件安装软件包，编辑 /etc/apt/sources.list，将 non-free-firmware 附加到每个 .debian.org 存储库行的末尾，然后运行 `apt update` 。

### SecureApt

存储库中的 Release 文件是使用 GnuPG 签名的。APT 使用这些签名来验证所有软件包是否来自受信任的来源。

如果从官方 ISO 映像安装 Proxmox VE，则已安装用于验证的密钥。

如果在 Debian 上安装 Proxmox VE，请使用以下命令下载并安装密钥：
...
Proxmox 定期为所有存储库提供更新。若要安装更新，请使用基于 Web 的 GUI 或以下 CLI 命令：
...
在裸机服务器上运行 Proxmox VE 时，应应用本章中的固件更新。在客户机中配置固件更新是否合适（例如，使用设备直通时）在很大程度上取决于您的设置，因此超出了范围。
...
> 如果更新说明需要重新启动主机，请确保可以安全地重新启动。

### Runtime Firmware Files 运行时固件文件
...
此方法将固件存储在 Proxmox VE 操作系统上，如果其持久固件较新，则将其传递给设备。网络和显卡等设备支持它，但依赖于持久固件（如主板和硬盘）的设备不支持它。

在 Proxmox VE 中，默认情况下已经安装了 pve-firmware 软件包。因此，在正常的系统更新（APT）中，常用硬件的固件会自动保持最新状态。

存在一个额外的  [Debian Firmware Repository](https://192.168.1.20:8006/pve-docs/pve-admin-guide.html#sysadmin_debian_firmware_repo) ，但缺省情况下未配置。
...
如果尝试安装其他固件包但冲突，APT 将中止安装。也许可以通过另一种方式获得特定的固件。
...
1. 启用 [Debian Firmware Repository](https://192.168.1.20:8006/pve-docs/pve-admin-guide.html#sysadmin_debian_firmware_repo) 。
2. `apt update` 获取最新的可用软件包（或使用 Web 界面，在 Node → Updates 下）
3. 安装特定于 CPU 供应商的微码包：
   - 对于 Intel CPU：`apt install intel-microcode`
   - 对于 AMD CPU：`apt install amd64-microcode`
4. 重新启动 Proxmox VE 主机
...
It makes sense to hold the downgraded package for a while and try more recent versions again at a later time. Even if the package version is the same in the future, system updates may have fixed the experienced problem in the meantime. 将降级的软件包保留一段时间并在以后再次尝试更新的版本是有意义的。即使将来的软件包版本相同，系统更新也可能在此期间修复了遇到的问题。  `# apt-mark hold intel-microcode intel-microcode set on hold.`  `# apt-mark unhold intel-microcode # apt update # apt upgrade`

## 网络配置

Proxmox VE 使用的是 Linux 网络堆栈。这为如何在 Proxmox VE 节点上设置网络提供了很大的灵活性。配置可以通过 GUI 完成，也可以通过手动编辑包含整个网络配置的文件 /etc/network/interfaces 来完成。interfaces(5) 手册页包含完整的格式描述。所有 Proxmox VE 工具都努力保持用户的直接修改，但使用 GUI 仍然是可取的，因为它可以保护您免受错误的影响。

需要 vmbr 接口将客户机连接到底层物理网络。它们是一个 Linux 网桥，可以将其视为客户机和物理接口连接到的虚拟交换机。本节提供了一些示例，说明如何设置网络以适应不同的用例，例如使用绑定、VLAN 或路由和 NAT 设置的冗余。

软件定义网络是 Proxmox VE 集群中更复杂的虚拟网络的一个选项。

> It’s discouraged to use the traditional Debian tools ifup and ifdown if unsure, as they have some pitfalls like interupting all guest traffic on ifdown vmbrX but not reconnecting those guest again when doing ifup on the same bridge later. 不鼓励使用传统的 Debian 工具 ifup 和 ifdown if unsure，因为它们存在一些陷阱，例如在 ifdown vmbrX 上中断所有来宾流量，但稍后在同一桥上执行 ifup 时不会再次重新连接这些来宾。

### 应用网络更改

Proxmox VE 不会将更改直接写入 /etc/network/interfaces 。取而代之的是写入一个名为 /etc/network/interfaces.new 的临时文件，这样就可以一次进行许多相关的更改。This also allows to ensure your changes are correct before applying, as a wrong network configuration may render a node inaccessible.还可以确保在应用之前更改正确，因为错误的网络配置可能会导致节点无法访问。
...
如果直接对 /etc/network/interfaces 文件进行了手动更改，则可以通过运行 ifreload -a 来应用它们。

> 如果在 Debian 上安装了 Proxmox VE，或者从较旧的 Proxmox VE 安装升级到 Proxmox VE 7.0，请确保安装了 ifupdown2：
>
...
- s<slot>[f<function>][n|d] — 按热插拔 ID 划分的设备
- [P<domain>]p<bus>s<slot>[f<function>][n|d] — 按总线 ID 划分的设备
- x<MAC> — 按 MAC 地址划分的设备

最常见的模式的一些示例包括：

- eno1 — 是第一个板载网卡
- enp3s0f1 — is function 1 of the NIC on PCI bus 3, slot 0  是 PCI 总线 3 插槽 0 上 NIC 的功能 1
...
根据当前的网络组织和资源，可以选择桥接、路由或伪装网络设置。

#### 专用局域网中的 Proxmox VE 服务器，使用外部网关访问互联网

在这种情况下，**Bridged** 模型最有意义，这也是新 Proxmox VE 安装的默认模式。每个 guest 系统都将有一个连接到 Proxmox VE 网桥的虚拟接口。这类似于将guest 网卡直接连接到 LAN 上的新交换机，Proxmox VE 主机扮演交换机的角色。

#### Proxmox VE server at hosting provider托管服务提供商的 Proxmox VE 服务器，为 guest 提供公共 IP 范围

对于此设置，可以使用 **Bridged** 模型或 **Routed** 模型，具体取决于提供商允许的内容。

#### Proxmox VE server at hosting provider托管服务提供商的 Proxmox VE 服务器，具有单个公共 IP 地址

在这种情况下，获取 guest 系统的传出网络访问权限的唯一方法是使用伪装 **Masquerading** 。对于对客户机的传入网络访问，您需要配置端口转发 **Port Forwarding** 。

为了进一步提高灵活性，可以配置 VLAN （IEEE 802.1q） 和网络绑定，也称为“链路聚合”。这样，就可以构建复杂而灵活的虚拟网络。

### 使用网桥的默认配置

 ![](../../../Image/d/default-network-setup-bridge.svg)

网桥就像在软件中实现的物理网络交换机。所有虚拟 guest 都可以共享一个网桥，也可以创建多个网桥来分隔网络域。每个主机最多可以有 4094 个网桥。
...
大多数托管服务提供商不支持上述设置。出于安全原因，一旦在单个接口上检测到多个 MAC 地址，它们就会禁用网络。

> Some providers allow you to register additional MACs through their management interface. This avoids the problem, but can be clumsy to configure because you need to register a MAC for each of your VMs. 某些提供商允许您通过其管理界面注册其他 MAC。这样可以避免该问题，但配置起来可能很笨拙，因为需要为每个 VM 注册一个 MAC。
...
您可以通过单个接口“路由”所有流量来避免此问题。这样可以确保所有网络数据包使用相同的MAC地址。
 ...
 ![](../../../Image/d/default-network-setup-routed.svg)

常见的情况是，有一个公共 IP（本示例假设为 198.51.100.5）和一个用于 VM 的额外 IP 块 （203.0.113.16/28）。对于此类情况，建议采用以下设置：
...
绑定（也称为 NIC  teaming 或链路聚合）是一种将多个 NIC 绑定到单个网络设备的技术。可以实现不同的目标，例如使网络容错、提高性能或两者兼而有之。

光纤通道等高速硬件和相关的交换硬件可能非常昂贵。通过执行链路聚合，两个 NIC 可以显示为一个逻辑接口，从而实现双倍速度。这是大多数交换机都支持的原生 Linux 内核功能。如果节点有多个以太网端口，可以通过将网线连接到不同的交换机来分配故障点，并且绑定连接将在出现网络故障时故障转移到一根电缆或另一根电缆。

聚合链路可以改善实时迁移延迟，并提高 Proxmox VE 集群节点之间的数据复制速度。

有 7 种模式：

- **Round-robin (balance-rr):** 轮询 Transmit network packets in sequential order from the first available network interface (NIC) slave through the last. 
   （balance-rr）：从第一个可用网络接口 （NIC） 从站到最后一个从站，按顺序传输网络数据包。此模式提供负载均衡和容错功能。
- **Active-backup (active-backup):** Only one NIC slave in the bond is active. A different slave becomes active if, and only if, the active slave fails. The single logical bonded interface’s MAC address is externally visible on only one NIC (port) to avoid distortion in the network switch.
  绑定中只有一个 NIC 从站处于活动状态。当且仅当活动从站发生故障时，其他从站才会变为活动对象。单个逻辑绑定接口的 MAC 地址仅在一个 NIC（端口）上外部可见，以避免网络交换机失真。此模式提供容错能力。
- **XOR (balance-xor):** Transmit network packets based on [(source MAC address XOR’d with destination MAC address) modulo NIC slave count]. This selects the same NIC slave for each destination MAC address.
  根据 [（源 MAC 地址 XOR'd 与目标 MAC 地址）模 NIC 从站计数]传输网络数据包。这将为每个目标 MAC 地址选择相同的 NIC 从站。此模式提供负载均衡和容错功能。
- **Broadcast (broadcast):** Transmit network packets on all slave network interfaces.
  在所有从网络接口上传输网络数据包。此模式提供容错能力。
- **IEEE 802.3ad Dynamic link aggregation (802.3ad)(LACP):** Creates aggregation groups that share the same speed and duplex settings. Utilizes all slave network interfaces in the active aggregator group according to the 802.3ad specification. 
  创建共享相同速度和双工设置的聚合组。根据 802.3ad 规范利用活动聚合器组中的所有从属网络接口。
...
如果交换机支持 LACP （IEEE 802.3ad） 协议，则建议使用相应的绑定模式 （802.3ad）。否则，通常应使用 active-backup 模式。

对于群集网络 （Corosync），建议将其配置为多个网络。Corosync 不需要网络冗余的绑定，因为如果网络无法使用，它可以自行在网络之间切换。
...
```
 ...
 ![](../../../Image/d/default-network-setup-bond.svg)

另一种可能性是直接使用 bond 作为桥梁端口。这可用于使访客网络具有容错能力。
...
每个 VLAN 网络都由一个通常称为标记 tag 的数字标识。然后，对网络包进行标记，以标识它们所属的虚拟网络。

#### Guest 网络的 VLAN

Proxmox VE 开箱即用地支持此设置。可以在创建 VM 时指定 VLAN 标记。VLAN 标记是 guest 网络配置的一部分。网络层支持不同的模式来实现 VLAN，具体取决于网桥配置：
...
Proxmox VE 在所有环境中都能正常工作，无论是否部署了 IPv6。建议将所有设置保留为提供的默认值。

如果仍然需要在节点上禁用对 IPv6 的支持，请通过创建适当的 sysctl.conf (5) 代码段文件并设置适当的 sysctl 来执行此操作，例如添加以下内容到  /etc/sysctl.d/disable-ipv6.conf ：
...
但在某些环境中，这可能是不希望的。从 Proxmox VE 7.3 开始，您可以通过在 “/etc/network/interfaces” 中的网桥上设置 “bridge-disable-mac-learning 1” 配置来禁用网桥上的 MAC 学习，例如：
...
启用后，Proxmox VE 将手动将VM和容器中配置的MAC地址添加到网桥转发数据库，以确保访客仍然可以使用网络 - 但前提是他们使用实际的MAC地址。

## 时间同步

Proxmox VE 集群堆栈本身在很大程度上依赖于所有节点都具有精确同步时间的事实。如果所有节点上的本地时间不同步，其他一些组件（如 Ceph）也将无法正常工作。
...
> 如果将系统升级到 Proxmox VE 7，建议手动安装 chrony、ntp 或 openntpd。

### 使用自定义 NTP 服务器

在某些情况下，可能需要使用非默认 NTP 服务器。例如，如果 Proxmox VE 节点由于限制性防火墙规则而无法访问公共互联网，则需要设置本地 NTP 服务器并告诉 NTP 守护程序使用它们。

#### 对于使用 chrony 的系统

应该在 /etc/chrony/chrony.conf 中指定 chrony 使用哪些服务器：
...
## External Metric Server 外部指标服务器

![](../../../Image/g/gui-datacenter-metric-server-list.png)

在 Proxmox VE 中，可以定义外部指标服务器，该服务器将定期接收有关主机，虚拟访客和存储的各种统计信息。
...
### Graphite 服务器配置
 ...
 ![](../../../Image/g/gui-datacenter-metric-server-graphite.png)

默认端口设置为 2003，默认 graphite 路径为 proxmox。

默认情况下，Proxmox VE 通过 UDP 发送数据，因此必须将 graphite 服务器配置为接受此数据。在这里，可以为不使用标准 1500 MTU 的环境配置最大传输单元 （MTU）。

In order not to block the important pvestatd statistic collection daemon, a timeout is required to cope with network problems.
还可以将插件配置为使用 TCP。为了不阻塞重要的 pvestatd 统计信息收集守护进程，需要超时来处理网络问题。

### Influxdb 插件配置

 ![](../../../Image/g/gui-datacenter-metric-server-influxdb.png)
...
使用此配置，服务器将侦听端口 8089 上的所有 IP 地址，并将数据写入 proxmox 数据库
...
大多数人直接在本地磁盘上安装 Proxmox VE。Proxmox VE 安装 CD 提供了多个本地磁盘管理选项，当前默认设置使用 LVM 。安装程序允许您为此类设置选择单个磁盘，并将该磁盘用作卷组 （VG）`pve` 的物理卷。以下输出来自使用小型 8GB 磁盘的测试安装：
...
- data
  ...
  此卷使用 LVM-thin，用于存储 VM 映像。LVM-thin 更适合此任务，因为它为快照和克隆提供了有效的支持。
...
强烈建议使用硬件 RAID 控制器（带 BBU）进行此类设置。这样可以提高性能，提供冗余，并使磁盘更换更容易（可热插拔）。

LVM 本身不需要任何特殊的硬件，对内存的要求非常低。

### Bootloader

默认情况下，安装两个引导加载程序。第一个分区包含标准的 GRUB 引导加载程序。第二个分区是 EFI 系统分区 （ESP），它可以在 EFI 系统上启动并从用户空间应用持久固件更新。

### 创建卷组

假设有一个空磁盘 /dev/sdb，在该磁盘上创建一个名为 “vmdata” 的卷组。
...
### Installation as Root File System 3.10.1. 作为根文件系统安装
...
### BTRFS Administration 3.10.2. BTRFS管理
...
### First Guest Boot Delay 3.11.4. First Guest Boot 延迟
...
如果您已经拥有要用于Proxmox VE节点的证书，则只需通过Web界面即可上传该证书。
 ...
 ![](../../../Image/g/gui-node-certs-upload-custom.png)

请注意，证书密钥文件（如果提供）不得受密码保护。

### 通过 Let's Encrypt （ACME） 获得的可信证书
...
#### ACME Account ACME账户
 ...
 ![](../../../Image/g/gui-datacenter-acme-register-account.png)
...
ACME 协议指定了不同类型的质询，例如 http-01，其中 Web 服务器提供具有特定内容的文件以证明它控制域。有时这是不可能的，要么是由于技术限制，要么是由于无法从公共 Internet 访问记录的地址。在这些情况下可以使用 dns-01 质询。通过在域区域中创建特定的 DNS 记录来满足此挑战。
 ...
 ![](../../../Image/g/gui-datacenter-acme-overview.png)
...
每个域都是特定于节点的。您可以在 Node -> Certificates 下添加新域条目或管理现有域条目，或者使用 pvenode config 命令。
 ...
 ![](https://pve.proxmox.com/pve-docs/images/screenshot/gui-node-certs-add-domain.png)
...
使用 DNS API 配置新插件的最简单方法是使用 Web 界面 （Datacenter -> ACME）。
 ...
 ![](../../../Image/g/gui-datacenter-acme-add-dns-plugin.png)
...
## Kernel Samepage Merging (KSM) 3.14. 内核同页合并（KSM）
...
### Implications of KSM 3.14.1. KSM的含义
