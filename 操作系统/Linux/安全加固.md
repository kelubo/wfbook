...
2. 经核查，CentOS操作系统仅采用用户名、口令一种方式对用户进行身份鉴别。
...
与您为保护服务器而设置的任何其他安全屏障一样，很坚韧防御对您的环境具有物理访问权限的人造成的无法估量的损害，例如硬盘被盗、电源或服务中断等。因此，控制台安全应仅作为整体物理安全策略的一个组成部分来解决。一个锁着的“屏蔽门”可能会阻止一个偶然的罪犯，或者至少会减慢一个坚定的罪犯的速度，所以建议在控制台安全方面采取基本的预防措施。 
...
任何能够接触到键盘的人都可以简单地使用Ctrl+Alt+Delete组合键重新启动服务器，而无需登录。虽然有人可以简单地拔掉电源，但您仍然应该防止在生产服务器上使用此组合键。这将迫使攻击者采取更激烈的措施来重新启动服务器，并同时防止意外重新启动。

To disable the reboot action taken by pressing the Ctrl+Alt+Delete key combination, run the following two commands:
要禁用通过按Ctrl+Alt+Delete组合键执行的重新启动操作，请运行以下两个命令：
