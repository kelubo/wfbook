...
现在我们仔细组建的系统看上去挺不错的，但是系统运行在多个层面上，所以匆忙间事情就可能出现问题。当硬盘出现故障或者分区受损(或者只是分区填满了)的时候，需要知道怎样很快和很容易地修好。管理员可以使用和前面进行初始配置一样的工具来维护系统，从故障中恢复。考虑一个硬盘发生故障的情况。因为RAID5提供了某种程度的数据冗余性，我们在上一节组建
