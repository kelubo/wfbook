...
选择发行版的建议：
- 服务器环境：首选 RHEL 或 Rocky Linux / AlmaLinux
- 开发环境：Ubuntu 或 Fedora（CentOS Stream 也可用于 RHEL 生态开发）
- 学习用途：Ubuntu 或 Rocky Linux / CentOS Stream
...
- 2020年：Red Hat 宣布停止 CentOS Linux，转向 CentOS Stream；Rocky Linux 和 AlmaLinux 相继诞生
...
实用技巧：
- 使用通配符批量操作：\texttt{rm *.log} 删除所有日志文件
- 使用绝对路径避免误操作：\texttt{rm -rf /data/logs/*}
- 使用 \texttt{-i} 选项确认删除：\texttt{rm -i important\_file.txt}
...
实用技巧：
- 使用 \texttt{top -p <pid>} 监控特定进程
- 使用 \texttt{free -w} 查看内存详细信息
- 使用 \texttt{du -sh * | sort -h} 按大小排序目录
...
常用用法：
- \texttt{ip addr show}：查看网络接口信息
- \texttt{ss -tnlp}：查看监听的TCP端口
- \texttt{netstat -rn}：查看路由表
...
SSD优化建议：
- 启用TRIM支持：\texttt{fstrim /mount/point}
- 使用noatime挂载选项
- 避免分区对齐问题
...
常用工具：
- \texttt{smartctl}：检查磁盘健康状态
- \texttt{iostat}：监控磁盘IO
- \texttt{badblocks}：检测坏道
...
用户信息存储位置：
- \texttt{/etc/passwd}：用户基本信息
- \texttt{/etc/group}：用户组信息
- \texttt{/etc/shadow}：用户密码（加密存储）
...
常见SUID程序：
- \texttt{/bin/passwd}：允许普通用户修改密码
- \texttt{/bin/su}：允许切换用户
- \texttt{/usr/bin/sudo}：允许提权执行命令
...
常用的参数文件：
- \texttt{/proc/sys/}：运行时内核参数
- \texttt{/etc/sysctl.conf}：系统启动时加载的内核参数
- \texttt{/etc/security/limits.conf}：用户资源限制
...
常用参数说明：
- \texttt{net.ipv4.tcp\_tw\_reuse}：允许复用TIME\_WAIT状态的socket
- \texttt{net.ipv4.tcp\_fin\_timeout}：FIN等待超时时间
- \texttt{vm.swappiness}：内存交换倾向（0-100）
- \texttt{fs.file-max}：系统最大文件描述符数
...
systemd服务单元文件位置：
- \texttt{/usr/lib/systemd/system/}：系统服务（不建议修改）
- \texttt{/etc/systemd/system/}：自定义服务
...
常用Vim插件：
- \textbf{Vundle}：插件管理器
- \textbf{NERDTree}：文件浏览器
- \textbf{YouCompleteMe}：代码补全
- \textbf{Airline}：状态栏美化
- \textbf{CtrlP}：快速文件查找
...
SDN架构组成：
- \textbf{控制平面}：集中式控制器，管理网络策略
- \textbf{数据平面}：交换机/路由器，执行数据转发
- \textbf{南向接口}：控制器与交换机通信（OpenFlow）
- \textbf{北向接口}：控制器与应用通信
...
Linux系统日志位置：
- \texttt{/var/log/messages}：系统消息日志
- \texttt{/var/log/auth.log}：认证日志
- \texttt{/var/log/syslog}：系统日志（Debian/Ubuntu）
- \texttt{/var/log/secure}：安全日志（RHEL/CentOS）
- \texttt{/var/log/nginx/}：Nginx日志
- \texttt{/var/log/mysql/}：MySQL日志
...
\subsection*{核心组件}
- \textbf{CDN加速}：分发静态资源，减轻源站压力
- \textbf{负载均衡集群}：多层负载均衡（L4 + L7）
- \textbf{分布式缓存}：Redis集群、Memcached集群
- \textbf{分布式数据库}：分库分表、读写分离
- \textbf{消息队列}：解耦异步处理（Kafka/RabbitMQ）
- \textbf{分布式存储}：对象存储（OSS）、分布式文件系统
...
\subsection*{负载均衡类型}
- \textbf{L4负载均衡}：基于IP和端口（LVS、F5）
- \textbf{L7负载均衡}：基于应用层协议（Nginx、HAProxy）
...
- \texttt{inter 2000}：检查间隔2秒
- \texttt{rise 2}：连续2次成功视为健康
- \texttt{fall 3}：连续3次失败视为不健康
...
\subsection*{配置参数说明}
- \texttt{state}：MASTER（主）或BACKUP（备）
- \texttt{virtual\_router\_id}：虚拟路由ID，主备必须相同
- \texttt{priority}：优先级，值越大越优先成为主服务器
- \texttt{advert\_int}：VRRP通告间隔（秒）
...
- \texttt{sync}：同步写入
- \texttt{async}：异步写入
- \texttt{no\_root\_squash}：允许root用户保持权限
- \texttt{root\_squash}：将root用户映射为匿名用户
...
- \textbf{Distribute}：分布式卷
- \textbf{Replica}：复制卷
- \textbf{Striped}：条带卷
- \textbf{Distributed-Replica}：分布式复制卷
- \textbf{Distributed-Striped}：分布式条带卷
...
\subsection*{Ceph组件}
- \textbf{Monitor}：管理集群状态
- \textbf{Manager}：管理集群监控和元数据
- \textbf{OSD}：对象存储守护进程
- \textbf{MDS}：元数据服务器（CephFS）
- \textbf{RGW}：对象网关（S3/Swift接口）
...
\subsection*{备份类型}
- \textbf{全量备份}：备份所有数据
- \textbf{增量备份}：备份上次备份后变化的数据
- \textbf{差异备份}：备份上次全量备份后变化的数据
...
\subsection*{CI/CD工具链}
- \textbf{持续集成}：Jenkins、GitLab CI、GitHub Actions
- \textbf{代码质量}：SonarQube、Codecov
- \textbf{容器化}：Docker、Kubernetes
- \textbf{部署工具}：Ansible、Terraform
...
- \textbf{插件管理}：安装Git、Ansible、Docker等插件
- \textbf{凭证管理}：管理SSH密钥、API密钥等
- \textbf{节点管理}：配置构建节点
...
\subsection*{组件说明}
- \textbf{Zabbix Server}：核心组件，负责数据收集、处理和存储
- \textbf{Zabbix Agent}：安装在被监控主机上，收集本地数据
- \textbf{Zabbix Proxy}：分布式监控代理，减轻Server压力
- \textbf{Zabbix Frontend}：Web界面，用于配置和查看监控数据
- \textbf{Database}：存储监控数据（MySQL/PostgreSQL/SQLite）
