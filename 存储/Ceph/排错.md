...
Cephadm 将守护进程部署为容器。这意味着对这些容器化的守护程序进行故障排除可能与您预期的工作方式不同。

以下是一些工具和命令，可帮助您排除Ceph环境的故障。

## 暂停或禁用 cephadm

如果出现问题并且 cephadm 表现不佳，可以通过运行以下命令暂停 Ceph 集群的大部分后台活动：

```bash
ceph orch pause
```

这将停止 Ceph 集群中的所有更改，但 cephadm 仍将定期检查主机以刷新其守护进程和设备的清单。可以通过运行以下命令完全禁用 cephadm：
...
这些命令将禁用所有的 `ceph orch ...` CLI命令。所有以前部署的守护程序容器将继续存在，并将按照运行这些命令之前的方式启动。
...
为了更轻松地调试失败的守护进程，cephadm 会存储每个服务和每个守护进程的事件。这些事件通常包含与 Ceph 集群故障排除相关的信息。
