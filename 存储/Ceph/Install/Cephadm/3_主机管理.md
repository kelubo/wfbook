...
The orchestrator supports assigning labels to hosts. Labels are free form and have no particular meaning by itself and each host can have multiple labels. They can be used to specify placement of daemons. See [Placement by labels](https://docs.ceph.com/en/latest/cephadm/service-management/#orch-placement-by-labels)
...
See also [Fully qualified domain names vs bare host names](https://docs.ceph.com/en/latest/cephadm/host-management/#cephadm-fqdn)
...
>
> cephadm要求通过ceph orch host add给出的主机名等于远程主机上主机名的输出。

Otherwise cephadm can’t be sure that names returned by `ceph * metadata` match the hosts known to cephadm. This might result in a [CEPHADM_STRAY_HOST](https://docs.ceph.com/en/latest/cephadm/operations/#cephadm-stray-host) warning.

否则cephadm无法确保ceph*元数据返回的名称与cephadm已知的主机匹配。这可能会导致CEPHADM流浪主机警告。
