...
以下命令打印编排器已知的服务列表。To limit the output to services only on a specified host要将输出限制为仅在指定主机上的服务，请使用可选的 `--host` 参数。To limit the output to services of only a particular type要将输出限制为仅特定类型的服务，请使用可选的 `--type` 参数（mon、osd、mgr、mds、rgw）：
...
Export the serviceto the orchestrator:要将服务 specs known 导出到编排器：

```bash
ceph orch ls --export
```

The service specifications exported with this command will be exported as yaml and that yaml c使用此命令导出的服务规范将导出为yaml，可以与 `ceph orch apply -i` 命令一起使用。

## 守护进程状态

A daemon is a systemd unit that is running and part of a service.守护程序是一个正在运行的系统单元，是服务的一部分。
...
服务规范是用于指定服务部署的数据结构。除了布局或网络等参数之外，用户还可以通过配置部分设置服务配置参数的初始值。对于每个参数 / 值配置对，cephadm 调用以下命令来设置其值：
...
This structure is supposed to be enough information这个结构应该是足够的信息来启动服务。

- `networks: List[str]`

  一个网络标识列表，指示守护进程仅绑定在该列表中的特定网络上。如果集群分布在多个网络上，则可以添加多个网络。请参见 [Networks and Ports](https://docs.ceph.com/en/latest/cephadm/services/monitoring/#cephadm-monitoring-networks-ports), [Specifying Networks](https://docs.ceph.com/en/latest/cephadm/services/rgw/#cephadm-rgw-networks) 和 [Specifying Networks](https://docs.ceph.com/en/latest/cephadm/services/mgr/#cephadm-mgr-networks) 。
...
- `service_type`
  ...
  服务的类型。Needs to be either a Ceph service (`mon`, `crash`, `mds`, `mgr`, `osd` or `rbd-mirror`), a gateway (`nfs` or `rgw`), part of the monitoring stack (`alertmanager`, `grafana`, `node-exporter` or `prometheus`) or (`container`) for custom containers.

  需要是Ceph服务（mon、crash、mds、mgr、osd或rbd镜像）、网关（nfs或rgw）、监视堆栈的一部分（alertmanager、grafana、节点导出器或prometheus）或自定义容器的（容器）。
...
- `unmanaged`
  ...
  如果设置为 true，则编排器将不会部署或删除与此服务关联的任何守护程序。放置和所有其他属性将被忽略。如果暂时不想管理此服务，这很有用。请参见 [Disable automatic deployment of daemons](https://docs.ceph.com/en/latest/cephadm/service-management/#cephadm-spec-unmanaged) 。
...
The Ceph Orchestrator maintains a declarative state of each service in a `ServiceSpec`. Ceph Orchestrator在服务规范中维护每个服务的声明性状态。对于某些操作，如更新 RGW HTTP 端口，我们需要更新现有规范。
...
> **Note:**
>
> **apply** command can be confusing.命令可能会令人困惑。因此，我们建议使用 YAML 规范。

每个 `ceph orch apply <service-name>` 命令将取代之前的命令。do not use the proper syntax, you will clobber your work as you go.如果你不使用正确的语法，你会在工作中出错。
...
（第一个命令在 host1 上创建一个 MON 。然后第二个命令在 host1 上删除 MON ，并在 host2 上创建 MON 。然后，第三个命令删除 host2 上的监视器，并创建 host3 上的MON 。在这种情况下，此时只有 host3 上有 MON 。）
...
enhanced network specifications:MON 和其他服务可能需要一些增强的网络规范：

```bash
ceph orch daemon add mon --placement="myhost:[v2:1.2.3.4:3300,v1:1.2.3.4:6789]=name"
```

其中 `[v2:1.2.3.4:3300,v1:1.2.3.4:6789]` 是 MON 的网络地址， `=name` 指定新监视器的名称。

### 按标签放置

守护程序的放置可以限于与特定标签匹配的主机。要将标签 mylabel 设置到适当的主机，请运行以下命令：
...
Cephadm’s declarative state consists of a list of service specifications containing placement specifications.的声明性状态由包含放置规范的服务规范列表组成。

Cephadm 不断将集群中实际运行的守护进程列表与服务规范中的列表进行比较。Cephadm 根据需要添加新的守护程序并删除旧的守护程序，以符合服务规范。

Cephadm 执行以下操作以保持符合服务规范。

Cephadm first selects a list of candidate hosts. 首先选择候选主机列表。Cephadm seeks explicit host names and selects them. 查找显式主机名并选择它们。如果 cephadm 没有找到明确的主机名，它会查找标签规范。如果规范中没有定义标签，cephadm 将根据主机模式选择主机。如果没有定义主机模式，作为最后的手段，cephadm 将选择所有已知的主机作为候选。
...
> cephadm 必须考虑一个特殊情况。
>
> 如果放置规范选择的主机少于计数要求的主机，则 cephadm 将仅部署在选定的主机上。
...
Cephadm支持在必要时为特定情况提供额外的杂项容器参数。例如，如果用户需要限制其mon守护进程使用的cpu数量，则可以应用如下规范
...
这将导致每个mon守护进程部署为--cpus=2。

### 装载带有额外容器参数的文件

A common use case for extra container arguments is to mount additional files within the container. However, some intuitive formats for doing so can cause deployment to fail (see https://tracker.ceph.com/issues/57338). The recommended syntax for mounting a file with extra container arguments is:

额外容器参数的一个常见用例是在容器中装载其他文件。然而，这样做的一些直观格式可能会导致部署失败（请参见https://tracker.ceph.com/issues/57338). 安装带有额外容器参数的文件的建议语法是：
...
Cephadm支持为守护程序指定其他配置文件。要做到这一点，用户必须提供配置文件的内容和守护程序容器中应该安装它的位置。在应用指定了自定义配置文件的YAML规范并让cephadm重新部署为其指定了配置文件的守护程序之后，这些文件将安装在守护程序容器中的指定位置。
...
To make these new config files actually get mounted within the containers for the daemons要使这些新的配置文件真正装入后台进程的容器中
...
>
> 在服务规范中应用此更改后，cephadm将不再部署任何新的守护程序（即使放置规范与其他主机匹配）。

### 在主机上手动部署守护程序

> Note
>
> This workflow has a very limited use case and should only be used in rare circumstances.此工作流的用例非常有限，只能在极少数情况下使用。

要在主机上手动部署守护程序，请执行以下步骤：

Modify the service spec for a service by getting the existing spec, adding , and applying the modified spec.通过获取现有规范、添加`unmanaged: true` ，并应用修改后的规范来修改服务。
...
>
> 从服务规范中删除unmanaged:true将启用此服务的协调循环，并可能导致删除守护程序，具体取决于放置规范。
