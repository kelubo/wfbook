...
默认情况，在所有使用 `_admin` 标签的主机上， `/etc/ceph` 中会有一个 `ceph.conf` 文件和一个 `client.admin` keyring 的副本。该标签最初仅应用于引导主机。建议为一个或多个其他主机提供 `_admin` 标签，以便 Ceph CLI（例如，通过`cephadm shell`）在多个主机上易于访问。将 `_admin` 标签添加到其他主机：
