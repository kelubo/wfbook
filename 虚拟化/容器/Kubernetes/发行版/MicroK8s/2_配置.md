# Configuring MicroK8s services 配置 MicroK8s 服务
...
要重新配置服务，您需要编辑相应的文件，然后重新启动 MicroK8s。例如，要将`调试`级别日志记录添加到 containerd：
...
#### `snap.microk8s.daemon-proxy` `snap.microk8s.守护程序代理`
...
#### `snap.microk8s.daemon-traefik` and `snap.microk8s.daemon-apiserver-proxy` `snap.microk8s.daemon-traefik` 和 `snap.microk8s.daemon-apiserver-proxy` 
...
# Configure host interfaces used by MicroK8s 配置 MicroK8s 使用的主机接口

#### On this page 本页内容

​                                                [Control Plane 控制平面](https://microk8s.io/docs/configure-host-interfaces#control-plane)                                                                                            [kube-apiserver kube-apiserver 服务器](https://microk8s.io/docs/configure-host-interfaces#kube-apiserver)                                                  [kube-controller-manager kube 控制器管理器](https://microk8s.io/docs/configure-host-interfaces#kube-controller-manager)                                                  [kube-scheduler kube-scheduler 调度器](https://microk8s.io/docs/configure-host-interfaces#kube-scheduler)                                                  [kube-proxy kube-proxy 代理](https://microk8s.io/docs/configure-host-interfaces#kube-proxy)                                                  [kubelet](https://microk8s.io/docs/configure-host-interfaces#kubelet)                                                  [dqlite](https://microk8s.io/docs/configure-host-interfaces#dqlite)                                                                                                    [Data Plane 数据平面](https://microk8s.io/docs/configure-host-interfaces#data-plane)                                                                                            [Calico VXLAN interface Calico VXLAN 接口](https://microk8s.io/docs/configure-host-interfaces#calico-vxlan-interface)                                                  [NodePort services NodePort 服务](https://microk8s.io/docs/configure-host-interfaces#nodeport-services)                                                                                                            

This document assumes basic knowledge of Kubernetes, and is only intended for advanced use cases.
本文档假定您具备 Kubernetes 的基本知识，并且仅适用于高级用例。
...
## [Control Plane 控制平面](https://microk8s.io/docs/configure-host-interfaces#control-plane)

This section explains how to configure the host interfaces used by the Kubernetes control plane services.
本节介绍如何配置 Kubernetes 控制平面服务使用的主机接口。

### [kube-apiserver kube-apiserver 服务器](https://microk8s.io/docs/configure-host-interfaces#kube-apiserver)
...
Apply the changes with: 使用以下方式应用更改：
...
### [kube-controller-manager kube 控制器管理器](https://microk8s.io/docs/configure-host-interfaces#kube-controller-manager)
...
### [kube-scheduler kube-scheduler 调度器](https://microk8s.io/docs/configure-host-interfaces#kube-scheduler)
...
### [kube-proxy kube-proxy 代理](https://microk8s.io/docs/configure-host-interfaces#kube-proxy)
...
### [kubelet](https://microk8s.io/docs/configure-host-interfaces#kubelet)
...
### [dqlite](https://microk8s.io/docs/configure-host-interfaces#dqlite)
...
## [Data Plane 数据平面](https://microk8s.io/docs/configure-host-interfaces#data-plane)

### [Calico VXLAN interface Calico VXLAN 接口](https://microk8s.io/docs/configure-host-interfaces#calico-vxlan-interface)
...
### [NodePort services NodePort 服务](https://microk8s.io/docs/configure-host-interfaces#nodeport-services)
...
# MicroK8s CNI Configuration MicroK8s CNI 配置

#### On this page 本页内容

​                                                [Configure Calico 配置 Calico](https://microk8s.io/docs/change-cidr#configure-calico)                                                                                            [Configure Calico IP autodetection method
配置 Calico IP 自动检测方法](https://microk8s.io/docs/change-cidr#configure-calico-ip-autodetection-method)                                                  [Configure Calico in BGP mode
在 BGP 模式下配置 Calico](https://microk8s.io/docs/change-cidr#configure-calico-in-bgp-mode)                                                  [Configure Pod CIDR 配置 Pod CIDR](https://microk8s.io/docs/change-cidr#configure-pod-cidr)                                                  [Upgrade Calico 升级 Calico](https://microk8s.io/docs/change-cidr#upgrade-calico)                                                                                                    [Flannel with Kubernetes store
Flannel 与 Kubernetes 存储](https://microk8s.io/docs/change-cidr#flannel-with-kubernetes-store)                                                      [Upgrade Flannel 升级法兰绒](https://microk8s.io/docs/change-cidr#upgrade-flannel)                                                              
...
在本文档中，我们将介绍默认的 MicroK8s CNI 配置，以及需要自定义设置时要遵循的步骤。
...
用于部署 Calico 的清单位于 `/var/snap/microk8s/current/args/cni-network/cni.yaml` 下。您可以在 [GitHub 上](https://github.com/canonical/microk8s/blob/1.26/upgrade-scripts/000-switch-to-calico/resources/calico.yaml)查看最新 MicroK8s 版本的源清单，也可以查看相应[分支](https://github.com/canonical/microk8s/blob/1.26/upgrade-scripts/000-switch-to-calico/resources/calico.yaml)中的特定版本（例如 1.26）的源清单
...
## [Configure Calico 配置 Calico](https://microk8s.io/docs/change-cidr#configure-calico)

For most of the cases below, the way to change Calico configuration is to patch the deployed `cni.yaml` and then re-apply it to the cluster with:
...
### [Configure Calico IP autodetection method 配置 Calico IP 自动检测方法](https://microk8s.io/docs/change-cidr#configure-calico-ip-autodetection-method)

Edit `/var/snap/microk8s/current/args/cni-network/cni.yaml` and change the following section:
...
### [Configure Calico in BGP mode 在 BGP 模式下配置 Calico](https://microk8s.io/docs/change-cidr#configure-calico-in-bgp-mode)
...
### [Configure Pod CIDR 配置 Pod CIDR](https://microk8s.io/docs/change-cidr#configure-pod-cidr)
...
### [Upgrade Calico 升级 Calico](https://microk8s.io/docs/change-cidr#upgrade-calico)
...
## [Flannel with Kubernetes store Flannel 与 Kubernetes 存储](https://microk8s.io/docs/change-cidr#flannel-with-kubernetes-store)
...
## [Upgrade Flannel 升级法兰绒](https://microk8s.io/docs/change-cidr#upgrade-flannel)
...
Starting from MicroK8s 1.25, it is possible to replace Calico with KubeOVN using the `kube-ovn` addon:
...
For more details, refer to the [KubeOVN addon documentation](https://microk8s.io/docs/addon-kube-ovn)
更多细节请参考 [KubeOVN 插件文档](https://microk8s.io/docs/addon-kube-ovn)
...
1. 部署集群时](https://microk8s.io/docs/add-launch-config#h-1-when-deploying-a-cluster)                                                      [2. Using snap set 2. 使用捕捉集](https://microk8s.io/docs/add-launch-config#h-2-using-snap-set)                                                      [3. Using a content snap
2. 使用内容快照](https://microk8s.io/docs/add-launch-config#h-3-using-a-content-snap)                                                              
...
### [1. When deploying a cluster 1. 部署集群时](https://microk8s.io/docs/add-launch-config#h-1-when-deploying-a-cluster)
...
### [2. Using `snap set` 2. 使用`捕捉集`](https://microk8s.io/docs/add-launch-config#h-2-using-snap-set)
...
### [3. Using a content snap 3. 使用内容快照](https://microk8s.io/docs/add-launch-config#h-3-using-a-content-snap)
...
在部署中自定义网络](https://microk8s.io/docs/how-to-dual-stack#customise-the-network-in-your-deployment)                                                      [Verify dual-stack is configured correctly
验证双堆栈配置是否正确](https://microk8s.io/docs/how-to-dual-stack#verify-dual-stack-is-configured-correctly)                                                      [Further Reading 延伸阅读](https://microk8s.io/docs/how-to-dual-stack#further-reading)                                                              
...
## [Further Reading 延伸阅读](https://microk8s.io/docs/how-to-dual-stack#further-reading)
...
- MicroK8s IPv6 DualStack on pre-1.28 releases [How to page](https://discuss.kubernetes.io/t/microk8s-ipv6-dualstack-how-to/14507)
  1.28 之前版本上的 MicroK8s IPv6 DualStack [作方法页面](https://discuss.kubernetes.io/t/microk8s-ipv6-dualstack-how-to/14507)

# MicroK8s with an external LMA 带外部 LMA 的 MicroK8s

#### On this page 本页内容

​                                                [Metrics 指标](https://microk8s.io/docs/external-lma#metrics)                                                                                            [kube-apiserver kube-apiserver 服务器](https://microk8s.io/docs/external-lma#kube-apiserverhttpskubernetesiodocsreferencecommand-line-tools-referencekube-apiserver)                                                  [kube-controller-manager kube 控制器管理器](https://microk8s.io/docs/external-lma#kube-controller-managerhttpskubernetesiodocsreferencecommand-line-tools-referencekube-controller-manager)                                                  [kube-proxy kube-proxy 代理](https://microk8s.io/docs/external-lma#kube-proxyhttpskubernetesiodocsreferencecommand-line-tools-referencekube-proxy)                                                  [kube-scheduler kube-scheduler 调度器](https://microk8s.io/docs/external-lma#kube-schedulerhttpskubernetesiodocsreferencecommand-line-tools-referencekube-scheduler)                                                  [kubelet](https://microk8s.io/docs/external-lma#kubelethttpskubernetesiodocsreferencecommand-line-tools-referencekubelet)                                                                                                    [Logs 原木](https://microk8s.io/docs/external-lma#logs)                                                      [Metrics Server Metrics 服务器](https://microk8s.io/docs/external-lma#metrics-server)                                                      [Prometheus 普罗 米修斯](https://microk8s.io/docs/external-lma#prometheus)                                                      [Alertmanager 警报管理器](https://microk8s.io/docs/external-lma#alertmanager)                                                              
...
- [Prometheus 普罗 米修斯](https://prometheus.io/)
- [Elasticsearch](https://www.elastic.co/elasticsearch/)
- [Alertmanager 警报管理器](https://prometheus.io/docs/alerting/latest/alertmanager/)
...
## [ Metrics 指标](https://microk8s.io/docs/external-lma#metrics)
...
### [ ](https://microk8s.io/docs/external-lma#kube-apiserverhttpskubernetesiodocsreferencecommand-line-tools-referencekube-apiserver)[*kube-apiserver kube-apiserver 服务器*](https://kubernetes.io/docs/reference/command-line-tools-reference/kube-apiserver/)
...
### [ ](https://microk8s.io/docs/external-lma#kube-controller-managerhttpskubernetesiodocsreferencecommand-line-tools-referencekube-controller-manager)[*kube-controller-manager kube 控制器管理器*](https://kubernetes.io/docs/reference/command-line-tools-reference/kube-controller-manager/)
...
### [ ](https://microk8s.io/docs/external-lma#kube-proxyhttpskubernetesiodocsreferencecommand-line-tools-referencekube-proxy)[*kube-proxy kube-proxy 代理*](https://kubernetes.io/docs/reference/command-line-tools-reference/kube-proxy/)
...
### [ ](https://microk8s.io/docs/external-lma#kube-schedulerhttpskubernetesiodocsreferencecommand-line-tools-referencekube-scheduler)[*kube-scheduler kube-scheduler 调度器*](https://kubernetes.io/docs/reference/command-line-tools-reference/kube-scheduler/)
...
### [ ](https://microk8s.io/docs/external-lma#kubelethttpskubernetesiodocsreferencecommand-line-tools-referencekubelet)[*kubelet*](https://kubernetes.io/docs/reference/command-line-tools-reference/kubelet/)
...
## [ Logs 原木](https://microk8s.io/docs/external-lma#logs)
...
# High level tools for monitoring, logging and alerting 用于监控、日志记录和警报的高级工具

## [ Metrics Server Metrics 服务器](https://microk8s.io/docs/external-lma#metrics-server)
...
## [ Prometheus 普罗 米修斯](https://microk8s.io/docs/external-lma#prometheus)
...
## [ Alertmanager 警报管理器](https://microk8s.io/docs/external-lma#alertmanager)
...
部署外部 etcd 集群](https://microk8s.io/docs/external-etcd#deploy-an-external-etcd-cluster)                                                      [Install MicroK8s 安装 MicroK8s](https://microk8s.io/docs/external-etcd#install-microk8s)                                                      [Switch MicroK8s to external etcd
将 MicroK8s 切换到外部 etcd](https://microk8s.io/docs/external-etcd#switch-microk8s-to-external-etcd)                                                      [That’s all! 就这样！](https://microk8s.io/docs/external-etcd#thats-all)                                                      [Notes for MicroK8s cluster
MicroK8s 集群说明](https://microk8s.io/docs/external-etcd#notes-for-microk8s-cluster)                                                              
