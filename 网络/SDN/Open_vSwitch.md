...
# 零依赖的软路由——使用OpenVSwitch改造家庭网络

众所周知，家庭网络少不了路由器。路由器是家庭网络的入口，承载了IP获取、DHCP、DNS、NAT等一系列基础功能。

常见的家庭网络拓扑中，路由器接在光猫后面，设备通过有线或无线的方式接在路由器后面，如下图所示：
...
家用路由器功能比较简单，说白了，家用路由器的核心功能不过就是广播域+DHCP Server+简单的SNAT网关+AP热点，这些功能跟网络上所说的“路由”其实关系不大。
由于家庭网络环境简单，设备少，也没有什么复杂的网络功能可以往里加。比较实用的附加功能其实也就是“一键组网”和“无线Mesh”，但是跟“路由”本身也没有什么太大关系。

而软路由则功能比较多，可定制性强。支持例如多拨号，限流，流量处理（例如去广告）等功能，性能富余的软路由还可以跑容器、虚拟机，或者当作NAS使用。此外软路由搭建简单方便，但凡是能装linux的设备，有两个网口（单网口稍复杂一些，后文会讲），就能做软路由，比如树莓派插usb网卡、笔记本电脑、HTPC等，也有一些硬路由可以刷OpenWrt等固件变成软路由。
...
其实我想要重新组网是因为看到了一款国产兆芯x86CPU的小机器。这款电脑使用UOS（Debian），外形也比较小巧。
最关键的是，它有两个网口。
...
这台电脑的CPU是兆芯，操作系统是UOS。国产CPU配国产操作系统，没毛病。UOS是deepin的商业版，而deepin是基于debian的。所以Debian能用的，UOS理论上都能用，Debian的使用方式大部分也能搬过来。
...
当时刚打开机器时还有几个更新项，我直接全给他更了。这里“更新”里面的项目可以全部禁用。不过其实它并不会自动安装。我干脆就一直开启了，在空闲时再VNC进去手动进行安装和重启。

![img](https://pic2.zhimg.com/80/v2-9aab220438f0915069c2ccffc13a5649_720w.webp)

更新设置可以全部关掉

更新其实用的也是apt。

![img](https://pic1.zhimg.com/80/v2-0401d438528b3c9130d57d81ffdbaa4c_720w.webp)

更新中

![img](https://pic1.zhimg.com/80/v2-95e7578f2ebff13cd7859825be643eb4_720w.webp)

更新后可能需要重启

正写这篇文章时候刚好检测到新补丁，更新了一把～
...
这里是重中之重。只有在root下才能装ovs，执行我们的SDN软路由配置。UOS允许开启root，但是注意！注意！注意！**一旦开启root，设备将失去保修！**

这其实也算是商家的一种自保手段。因为root后，debian能干的事UOS都能干。就不说`rm -rf /`了，刷个固件啥的玩坏也都是有可能的。对这种约束表示理解。

在“控制中心->通用->开发者模式”里，可以“进入开发者模式”。点击后会弹出确认框。接下来需要注册一个UOS账号，登录账号后才能进入开发者模式。好在注册流程比较简单，也没啥花里胡哨的限制。

![img](https://pic4.zhimg.com/80/v2-9a57673a1bd7785870278ad94297069f_720w.webp)
...
​                    [ludongguoa](https://blog.csdn.net/ludongguoa)                    ![img](https://csdnimg.cn/release/blogv2/dist/pc/img/newCurrentTime2.png)                    于 2021-11-03 15:37:43 发布                    ![img](https://csdnimg.cn/release/blogv2/dist/pc/img/articleReadEyes2.png)                    2835                                            ![img](https://csdnimg.cn/release/blogv2/dist/pc/img/tobarCollect2.png)                                                收藏                                                    19                                                                

​                            文章标签：                                [网络](https://so.csdn.net/so/search/s.do?q=网络&t=blog&o=vip&s=&l=&f=&viparticle=)                                [linux](https://so.csdn.net/so/search/s.do?q=linux&t=blog&o=vip&s=&l=&f=&viparticle=)                                [网络协议](https://so.csdn.net/so/search/s.do?q=网络协议&t=blog&o=vip&s=&l=&f=&viparticle=)                    

​                    版权                

​                        [                          ![img](https://devpress.csdnimg.cn/489fad64a62648818eaaebc28e5c8659.jpg)                                                                                      华为云开发者联盟                              该内容已被华为云开发者联盟社区收录，社区免费抽好礼🎁，最高可得Switch!                                                                                ](javascript:;)                      
...
这还要从一个概念[SDN](https://so.csdn.net/so/search?q=SDN&spm=1001.2101.3001.7020)说起，全称Software Defined Network，软件定义网络。
