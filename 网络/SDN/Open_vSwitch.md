...
由于 DPDK 只是一个库，它本身并不能做很多事情，因此它依赖于使用它的新兴项目。已经是 Ubuntu 一部分的库的一个使用者是 Open vSwitch 在包 `openvswitch-switch-dpdk` 中支持 DPDK （OvS-DPDK） 。
...
下面是一个简短的示例，说明如何使用 DPDK 安装和配置基本的 Open vSwitch，以便以后通过 `libvirt` / `qemu-kvm` 使用。
...
> 在重新启动之前，您需要将设备分配给与 DPDK 兼容的驱动程序 - 请参阅有关取消分配默认内核驱动程序的 DPDK 部分。
...
请注意，上述示例中的部分是 `_dpdk-alloc-mem=2048_` 单插槽系统最基本的非一致性内存访问 （NUMA） 设置。如果有多个套接字，则可能需要定义如何在它们之间分配内存。有关这些选项的更多详细信息，请参阅 Open vSwitch 设置。

## Attach DPDK ports to Open vSwitch 将 DPDK 端口连接到 Open vSwitch
...
上面启动的 Open vSwitch 支持与 Open vSwitch 通常相同的所有端口类型，以及 DPDK 端口类型。以下示例演示如何创建网桥，并向其添加外部 DPDK 端口（而不是普通外部端口）。执行此操作时，您可以指定关联的设备。
...
## Open vSwitch DPDK to KVM guests 打开 vSwitch DPDK 到 KVM 客户机
...
如果您不在 DPDK 之上构建某种软件定义网络 （SDN） 交换机或 NFV，则很可能希望将流量转发到 KVM 客户机。好消息是;使用 Ubuntu 中的新 `qemu` / `libvirt` / `dpdk` / 版本 `openvswitch` ，这不再是手动附加命令行字符串。本节演示了将 KVM 客户机连接到 Open vSwitch DPDK 实例的基本设置。
...
访问 KVM 客户机的推荐方法是使用 `vhost_user_client` .这将导致 OvS-DPDK 连接到 QEMU 创建的套接字。通过这种方式，我们可以避免诸如“OvS 重启时客户机故障”之类的老问题。下面是一个示例，说明如何将此类端口添加到您上面创建的网桥。
...
要允许 `libvirt` / `kvm` 使用此套接字并为其创建客户机 VirtIO 网络设备，请将以下代码片段作为网络定义添加到您的客户机定义中。
...
第一行选择每个 DPDK 接口要使用的 Rx 队列数，而第二行控制要运行的轮询模式驱动程序 （PMD） 线程数（以及运行它们的位置）。上面的示例将使用两个 Rx 队列，并在 CPU 1 和 2 上运行 PMD 线程。
...
> 有关详细信息，请查看本文档末尾的“EAL 命令行选项”和“打开 vSwitch DPDK 安装”的链接。
...
与通常的调优一样，您需要非常了解您的系统和工作负载 - 因此，请验证任何与实际用例相匹配的工作负载的调优。

## Support and troubleshooting 支持和故障排除
...
问题通常是由于常规设置中缺少小细节造成的。后来，这些缺失的细节会导致难以追踪其根本原因的问题。
...
一个常见的情况似乎是“无法打开网络设备 dpdk0（没有此类设备）”问题。在使用 DPDK 的 Open vSwitch  中设置端口时，这种情况发生得相当晚，但根本原因（大多数情况下）是在设置和初始化的早期。以下是设备的正确初始化外观的示例 - 可以在启用 DPDK `syslog/journal` 的情况下启动 Open vSwitch 时找到。
...
如果由于卡被忽略、初始化失败或其他原因而缺少此功能，则以后将没有 DPDK 设备可供参考。不幸的是，日志记录分布在日志中 `syslog/journal`  `openvswitch` 。为了启用一些交叉检查，下面是一个示例，说明在这些日志中可以找到的内容，相对于输入的命令。
...
- [DPDK documentation DPDK 文档](http://dpdk.org/doc)
- [Release Notes matching the version packages in Ubuntu 16.04
  与 Ubuntu 16.04 中的版本包匹配的发行说明](http://dpdk.org/doc/guides/rel_notes/release_2_2.html)
- [Linux DPDK user getting started
  Linux DPDK 用户入门](http://dpdk.org/doc/guides/linux_gsg/index.html)
- [EAL command-line options
  EAL 命令行选项](http://dpdk.org/doc/guides/testpmd_app_ug/run_app.html)
- [DPDK API documentation DPDK API 文档](http://dpdk.org/doc/api/)
- [Open Vswitch DPDK installation
  打开 Vswitch DPDK 安装](https://github.com/openvswitch/ovs/blob/branch-2.5/INSTALL.DPDK.md)
