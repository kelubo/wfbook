...
- ​							要在不同的区中工作，请使用 `--zone=*<zone_name>*` 选项。例如，要允许在区 `public` 中使用 `SSH` 服务： 					
...
2. ​							确保您要在它们之间启用区内转发的接口没有分配给不是 `internal` 区的区： 					
...
- ​							[使用 nftables 命令中的设置](https://access.redhat.com/documentation/zh-cn/red_hat_enterprise_linux/9/html/configuring_firewalls_and_packet_filters/getting-started-with-nftables_firewall-packet-filters#using-sets-in-nftables-commands_getting-started-with-nftables) 					
- ​							[在 nftables 命令中使用 verdict 映射](https://access.redhat.com/documentation/zh-cn/red_hat_enterprise_linux/9/html/configuring_firewalls_and_packet_filters/getting-started-with-nftables_firewall-packet-filters#using-verdict-maps-in-nftables-commands_getting-started-with-nftables) 					
...
- ​							[将特定本地端口上传入的数据包转发到不同主机](https://access.redhat.com/documentation/zh-cn/red_hat_enterprise_linux/9/html-single/configuring_firewalls_and_packet_filters/index#forwarding-incoming-packets-on-a-specific-local-port-to-a-different-host_configuring-port-forwarding-using-nftables) 					

### 2.4.4. 使用 nftables 配置目标 NAT

​					目的地 NAT (DNAT)使您能够将路由器上的流量重定向到无法从互联网访问的主机。 			
...
3. ​							向 `prerouting` 链中添加一条规则，该规则将到路由器的 `ens3` 接口上端口 `80` 和 `443` 的传入流量重定向到 IP 地址为 `192.0.2.1` 的 Web 服务器： 					
...
- ​							[NAT 类型](https://access.redhat.com/documentation/zh-cn/red_hat_enterprise_linux/9/html-single/configuring_firewalls_and_packet_filters/index#nat-types_configuring-nat-using-nftables) 					
...
​					匿名集合的缺陷是，如果要更改集合，则需要替换规则。对于动态解决方案，使用命名集合，如 [在 nftables 中使用命名集合](https://access.redhat.com/documentation/zh-cn/red_hat_enterprise_linux/9/html-single/configuring_firewalls_and_packet_filters/index#using-named-sets-in-nftables_using-sets-in-nftables-commands) 中所述。 			
...
​					匿名映射的缺点是，如果要修改映射，则必须替换规则。对于动态解决方案，请使用命名映射，如 [在 nftables 中使用命名映射](https://access.redhat.com/documentation/zh-cn/red_hat_enterprise_linux/9/html-single/configuring_firewalls_and_packet_filters/index#using-named-maps-in-nftables_using-verdict-maps-in-nftables-commands) 中所述。 			
...
​				使用 RHEL 路由器上的 `nftables` 框架来编写和安装防火墙脚本，该脚本可保护内部 LAN 中的网络客户端以及 DMZ 中的 Web 服务器，使其免受来自互联网和其他网络的未授权访问。 		
...
​					本例是运行在 RHEL 路由器上的一个 `nftables` 防火墙脚本，其保护内部 LAN 中的客户端以及 DMZ 中的 Web 服务器。有关示例中使用的防火墙的网络和要求的详情，请参阅 [网络条件](https://access.redhat.com/documentation/zh-cn/red_hat_enterprise_linux/9/html-single/configuring_firewalls_and_packet_filters/index#con_network-conditions_assembly_example-protecting-a-lan-and-dmz-using-an-nftables-script) 和 [对防火墙脚本的安全要求](https://access.redhat.com/documentation/zh-cn/red_hat_enterprise_linux/9/html-single/configuring_firewalls_and_packet_filters/index#con_security-requirements-to-the-firewall-script_assembly_example-protecting-a-lan-and-dmz-using-an-nftables-script) 。 			
...
- ​							网络已配置，如 [网络条件](https://access.redhat.com/documentation/zh-cn/red_hat_enterprise_linux/9/html-single/configuring_firewalls_and_packet_filters/index#con_network-conditions_assembly_example-protecting-a-lan-and-dmz-using-an-nftables-script) 中所述。 					
...
- ​							[在 nftables 中使用命名集](https://access.redhat.com/documentation/zh-cn/red_hat_enterprise_linux/9/html-single/configuring_firewalls_and_packet_filters/index#using-named-sets-in-nftables_using-sets-in-nftables-commands) 					
...
- ​							有关向现有规则添加计数器的流程的更多信息，请参阅 [向现有规则添加计数器](https://access.redhat.com/documentation/zh-cn/red_hat_enterprise_linux/9/html-single/configuring_firewalls_and_packet_filters/index#adding-a-counter-to-an-existing-rule_debugging-nftables-rules)。 					
...
- ​							有关添加带有计数器的新规则的流程的更多信息，请参阅 [创建带有计数器的规则](https://access.redhat.com/documentation/zh-cn/red_hat_enterprise_linux/9/html-single/configuring_firewalls_and_packet_filters/index#creating-a-rule-with-a-counter_debugging-nftables-rules) 。 					
...
- ​						[在 Red Hat Enterprise Linux 8 中使用 nftables](https://www.redhat.com/en/blog/using-nftables-red-hat-enterprise-linux-8) 				
- ​						[iptables 之后是什么？当然，它的继任者是：nftables](https://developers.redhat.com/blog/2016/10/28/what-comes-after-iptables-its-successor-of-course-nftables/) 				
- ​						[firewalld:nftables 是将来的选择](https://developers.redhat.com/blog/2018/08/10/firewalld-the-future-is-nftables/) 				
...
​		The text of and illustrations in this document are licensed by Red Hat under a Creative Commons Attribution–Share Alike 3.0 Unported license  ("CC-BY-SA"). An explanation of CC-BY-SA is available at [http://creativecomm](http://creativecommons.org/licenses/by-sa/3.0/)
...
iptables 防火墙是静态的，每次修改都要求防火墙完全重启，这个过程包括内核 netfilter 防火墙模块的卸载和新配置所需模块的装载等，而模块的卸载将会破坏状态防火墙和确立建立的连接。
...
> 注意:
>
> 带有 `nftables` 后端的 `firewalld` 不支持使用 `--direct` 选项将自定义的 `nftables` 规则传递到 `firewalld`。
>
> firewalld 与 iptables、nftables 等一样，本身并不是防火墙，只是防火墙管理工具，通过定义防火墙规则最终交由内核的 netfilter 进行包过滤实现防火墙功能。

firewalld 软件包提供了 firewalld daemon 和一系列工具如 firewall-cmd、firewall-config、firewall-offline-cmd 等。

firewalld 由 d-bus 层和 core 层构成。

* d-bus 层提供 api，其它程序（如 firewall-cmd、firewall-config、firewall-applet、甚至是自己编写的程序或守护进程）通过调用 api 可定义防火墙规则或查询防火墙状态等等。
* core 层为 d-bus 层提供服务，负责和后端交互。

 ![](../../Image/f/firewalld.png)

> 备注：
>
> firewalld提供的firewall-offline-cmd不调用d-bus层，而是直接调用core层中的IO来修改防火墙规则。
...
zones 代表一种更透明管理传入流量的概念。这些区域连接到联网接口或者分配一系列源地址。可以独立为每个区管理防火墙规则，这样就可以定义复杂的防火墙设置并将其应用到流量。

zone 是 firewalld 一切的基础，在 zone 中可配置各种规则，比如根据 service / port（经常使用）、source 等定义，或定义 rich  rule（富规则），所以说 zone 是预先定义的一组规则集。这样 zone 就实现了对网络可信等级的划分。firewalld 缺省定义了9个 zone（当然可以对其可修改，也可以自己创建额外的 zone）。
...
通过对网卡（实际上是connection，即网卡配置文件。虽然一个网卡可有多个配置文件，但同时只能有一个活动的配置文件，可以在配置文件中使用 `zone=` 来定义 connection 所属 zone）切换 zone，可快速将网络置于不同的可信等级。

此外，zone 也提供了类似路由器一些功能，如端口转发、ip地址伪装等。
...
这三个的优先级按顺序依次降低，也就是说如果按照 source 可以找到就不会再按 interface 去查找，如果前两个都找不到才会使用第三个，也就是 firewalld.conf中配置的默认 zone 。

 ![](../../Image/f/firewalld_1.png)
...
可以使用图形化的 `firewall-config` 工具、`firewall-cmd` 和 `firewall-offline-cmd` 来添加和删除服务。也可以编辑 `/etc/firewalld/services/` 目录中的 XML 文件。

如果用户未添加或更改服务，则在 `/etc/firewalld/services/` 中没有相应的 XML 文件。如果要添加或更改服务，`/usr/lib/firewalld/services/` 目录中的文件可作用作模板。 
...
如果永久配置有效，该命令将返回 `success`。在其他情况下，命令返回一个带有更多详情的错误，如下所示： 					
...
2. 要指定显示设置的区，在 `firewall-cmd--list-all` 命令中添加 `--zone=zone-name` 参数
...
3. 使用命令帮助来查看 `firewalld` 手册页或获取选项列表
...
2. 使用本地文件添加新服务： 					
   ...
   可以使用 `--name=service-name` 选项来更改服务名称。 					

3. 更改服务设置后，服务的更新副本放在 `/etc/firewalld/services/` 中。 `root` 用户，可以输入以下命令来手动复制服务： 					
...
端口是可让操作系统接收和区分网络流量并将其转发到系统服务的逻辑设备。通常由侦听端口的守护进程来表示，它会等待到达这个端口的任何流量。通常，系统服务侦听为它们保留的标准端口。
...
当打开的端口不再需要时，在 `firewalld` 中关闭此端口。强烈建议尽快关闭所有不必要的端口，因为端口处于打开状态会存在安全隐患。
...
描述如何使用 `nmcli` 工具将 `firewalld` 区添加到 `NetworkManager` 连接中。
...
要使用自定义区，创建一个新的区并使用它像预定义区一样。新区需要 `--permanent` 选项。
...
## Managing Ports[¶](https://docs.rockylinux.org/zh/guides/security/firewalld-beginners/#managing-ports)
...
## Service management commands[¶](https://docs.rockylinux.org/zh/guides/security/firewalld-beginners/#service-management-commands)
...
## Restricting Access[¶](https://docs.rockylinux.org/zh/guides/security/firewalld-beginners/#restricting-access)
...
## Final Notes[¶](https://docs.rockylinux.org/zh/guides/security/firewalld-beginners/#final-notes)
...
This guide focuses on applying rules from an `iptables` firewall to a `firewalld` firewall. If you are really at the beginning of your firewall journey, [this document](https://docs.rockylinux.org/zh/guides/security/firewalld-beginners/) might help you more. Consider reading through both documents to get the most out of `firewalld`.

## Prerequisites and Assumptions[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#prerequisites-and-assumptions)
...
## Zones[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#zones)
...
### Adding Zones[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#adding-zones)
...
### Listing Zones[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#listing-zones)
...
### Removing an IP and Service from a Zone[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#removing-an-ip-and-service-from-a-zone)
...
### Using A New Zone - Adding Administrative IPs[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#using-a-new-zone-adding-administrative-ips)
...
## ICMP Rules[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#icmp-rules)
...
## Web Server Ports[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#web-server-ports)
...
## FTP Ports[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#ftp-ports)
...
This portion of the script deals with the standard FTP ports (20 and  21) as well as opening some additional passive ports. This sort of a  rule set is often needed by such ftp servers as [VSFTPD](https://docs.rockylinux.org/zh/guides/file_sharing/secure_ftp_server_vsftpd/). Generally, this sort of rule would be on a publicly facing web server,  and is there for allowing ftp connections from your customers.
...
## Database Ports[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#database-ports)
...
### Postgresql Considerations[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#postgresql-considerations)
...
## DNS Ports[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#dns-ports)
...
## More on Listing Rules[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#more-on-listing-rules)
...
## Establised Related Rules[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#establised-related-rules)
...
## Interfaces[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#interfaces)
...
## Common firewall-cmd Commands[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#common-firewall-cmd-commands)
...
## Conclusion[¶](https://docs.rockylinux.org/zh/guides/security/firewalld/#conclusion)
